...
└── vector_db/            # 向量数据库目录（执行 clear 脚本时自动创建）
```

## 📂 数据目录说明

本项目自带一个用于演示与研究的 `data` 目录（例如 `data/recipes/`），其中包含若干甜品的 JSON 数据。为便于展示基于私有资料库进行问答的示例，项目还同时加入了一些用于演示的“伪造甜品”条目，例如：
